    ...
    with torch.no_grad():
        for text, cnt in tqdm(data_iter):
            if args.image2video:
                ...
                image_2 = image_2 * 2.0 - 1.0
                image_2 = image_2.unsqueeze(2).to(torch.bfloat16)
                image_2 = model.encode_first_stage(image_2, None)
                image_2 = image_2.permute(0, 2, 1, 3, 4).contiguous()

                pad_shape = (image.shape[0], T - 2, C, H // F, W // F)
                image = torch.cat([image, torch.zeros(pad_shape).to(image.device).to(image.dtype), image_2], dim=1)
            ...
            for index in range(args.batch_size):
                ...
                ground_truth = video_reader[0: ((T - 1) * 4 + 1) * 2: 2].asnumpy() # (f, h, w, c)
